\chapter{Using Effects and Handlers to Implement a Game Engine With ECS}\label{chap:details}
...
This principle of storing the component data in an existential type also allows setting a component value without the relevant |Component| effect handled at the call site, which is used later for deferred modification in \Cref{sec:entitymanager}.
...
They way it currently is laid out, creating additional named handlers for any components that are needed in the query, is the result of multiple iterations. After a rework of the ECS, the idea was to pack all of the component types that get queried inside the single |Query| effect. This would allow adding to the query without creating named handlers and iterating over the components themselves later, instead of just the Entities and getting/setting the components via the named handler. This approach, however, did not work as intended. That is mainly because of the current lack of "recursive bidirectional effects", which is requiring the effect itself inside an effect operation as a bidirectional effect. This means, calling the effect operation of a named handler would need another handler of the same effect in scope at the call site, which is currently not possible. After trying different concepts for the query implementation, this is the one that worked and was relatively ergonomic to use.
